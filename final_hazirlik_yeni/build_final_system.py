...
def write_summaries():
    ...
    critical = ["# Final Icin En Kritik 50 Bilgi", ""]
    source_topics = TOPICS + TOPICS[:15]
    for idx, (_, title, definition, source, tags) in enumerate(source_topics[:50], start=1):
        critical.append(f"## {idx}. {title}")
        critical.append(f"- Kisa bilgi: {definition}")
        critical.append(f"- Neden onemli?: {'; '.join(tags[:3])} basliklari finalde dogrudan yorum sorusuna donusebilir.")
        critical.append("- Hangi soru tipinde cikar?: Test, kod yorumlama veya klasik deger takip sorusu.")
        critical.append(f"- Hatirlatma: Kaynak notu: {source}.")
        critical.append("")

    predicted = ["# Hoca Tarzi Tahmini Sorular", "", "Bu dosya, prompttaki sinav beklentisine gore final sabahi prova amaciyla hazirlanmistir.", ""]
    predicted.append("## 30 Test Sorusu")
    for i, (_, title, definition, source, tags) in enumerate((TOPICS * 2)[:30], start=1):
        predicted.append(f"{i}. {title}: {definition} Bu konu hangi asamada belirlenir ve sinav tuzagi nedir? Kaynak: {source}")
    predicted.append("\n## 10 Klasik Soru")
    for i, name in enumerate(["Pass by value", "Pass by reference", "Pass by value-result", "Pass by name", "Static scope", "Dynamic scope", "Activation record", "Closure", "Coroutine", "Dynamic binding"], start=1):
        predicted.append(f"{i}. {name} icin kisa kod verildiginde degiskenlerin son degerlerini, binding zamanini ve yaygin hatayi acikla.")
    predicted.append("\n## 10 Kod Parcasindan Dil Tanima Sorusu")
    for i, lang in enumerate(list(LANGS)[:10], start=1):
        predicted.append(f"{i}. {lang}: Ayirt edici syntax ipucunu, karistirilabilecek dili ve en guclu kaniti yaz.")
    predicted.append("\n## 10 Kod Yorumlama / Deger Takip Sorusu")
    for i, name in enumerate(["aliasing", "side effect", "recursive call stack", "closure capture", "Go channel blocking", "Rust move", "Scala match", "Erlang receive", "exception propagation", "short-circuit"], start=1):
        predicted.append(f"{i}. {name}: kodun hangi satirda hangi degeri urettigini ve hatanin compile-time mi run-time mi oldugunu belirt.")

    project = ["# Proje Dilleri Ozeti", "", "Kaynak: Ders Sunumlari.zip icindeki proje raporlari ve genel programlama dilleri bilgisiyle guclendirilen sinav notlari.", ""]
    for lang in ["Rust", "Go", "Scala", "Erlang"]:
        purpose, types, memory, conc, syntax, trap = LANGS[lang]
        project.extend([
            f"## {lang}",
            f"- Ortaya cikis amaci: {purpose}.",
            ...
            f"- Paradigma: {purpose}.",
            f"- Tip sistemi: {types}.",
            f"- Bellek yonetimi: {memory}.",
            f"- Concurrency yaklasimi: {conc}.",
            f"- Syntax ayirt edicileri: {syntax}.",
            ...
            f"- Diger dillerden farki: {trap}",
            "- Finalde nasil sorulabilir?: Kod parcasi verilip dil tanima, concurrency modeli veya bellek yonetimi sorulabilir.",
            f"- Acik uclu soru ornegi: {lang} dilinin concurrency ve tip/bellek modeli sinavdaki klasik kavramlarla nasil baglanir?",
            f"- Test soru ornegi: {lang} kodunu tanimak icin en guclu ipucu hangisidir?",
            "- Kaynak: Proje raporlari; eksik ayrinti varsa Genel bilgi.",
            "",
        ])

    extra_docs = {
        "06_FINAL_ICIN_EN_KRITIK_50_BILGI.md": critical,
        "07_HOCA_TARZI_TAHMINI_SORULAR.md": predicted,
        "08_PROJE_DILLERI_OZETI.md": project,
    }
    for filename, lines in extra_docs.items():
        (DIRS["ozet"] / filename).write_text("\n".join(lines), encoding="utf-8")
...
<body>
  ...
  <main class="layout">
    <aside class="panel">
      ...
      <label><input type="checkbox" id="unsolvedOnly"> Cozulmemisler</label>
...
const FILTER_KEY="pd-final-filters-v2";
const savedFilters=JSON.parse(localStorage.getItem(FILTER_KEY)||"{}");
const state={search:savedFilters.search||"",type:savedFilters.type||"",difficulty:savedFilters.difficulty||"",tags:new Set(savedFilters.tags||[]),favOnly:!!savedFilters.favOnly,wrongOnly:!!savedFilters.wrongOnly,unsolvedOnly:!!savedFilters.unsolvedOnly,progress:JSON.parse(localStorage.getItem(STORE_KEY)||"{}")};
...
function saveFilters(){localStorage.setItem(FILTER_KEY,JSON.stringify({search:state.search,type:state.type,difficulty:state.difficulty,tags:[...state.tags],favOnly:state.favOnly,wrongOnly:state.wrongOnly,unsolvedOnly:state.unsolvedOnly}))}
...
function filtered(){const term=state.search.trim().toLocaleLowerCase("tr");return QUESTIONS.filter(q=>{const p=state.progress[q.id]||{};const hay=[q.id,q.topic,q.question,q.source,q.code||"",q.expected||"",...(q.tags||[])].join(" ").toLocaleLowerCase("tr");if(term&&!hay.includes(term))return false;if(state.type&&q.type!==state.type)return false;if(state.difficulty&&q.difficulty!==state.difficulty)return false;if(state.favOnly&&!p.favorite)return false;if(state.wrongOnly&&p.status!=="wrong")return false;if(state.unsolvedOnly&&p.solved)return false;for(const t of state.tags)if(!q.tags.includes(t))return false;return true})}
...
function setup(){allDiffs.forEach(d=>$("difficultyFilter").innerHTML+=`<option value="${d}">${d}</option>`);$("tagFilters").innerHTML=allTags.map(t=>`<button class="chip ${state.tags.has(t)?"active":""}" data-tag="${t}">${t}</button>`).join("");$("searchBox").value=state.search;$("typeFilter").value=state.type;$("difficultyFilter").value=state.difficulty;$("favOnly").checked=state.favOnly;$("wrongOnly").checked=state.wrongOnly;$("unsolvedOnly").checked=state.unsolvedOnly;$("searchBox").oninput=e=>{state.search=e.target.value;saveFilters();render()};$("typeFilter").onchange=e=>{state.type=e.target.value;saveFilters();render()};$("difficultyFilter").onchange=e=>{state.difficulty=e.target.value;saveFilters();render()};$("favOnly").onchange=e=>{state.favOnly=e.target.checked;saveFilters();render()};$("wrongOnly").onchange=e=>{state.wrongOnly=e.target.checked;saveFilters();render()};$("unsolvedOnly").onchange=e=>{state.unsolvedOnly=e.target.checked;saveFilters();render()};$("tagFilters").onclick=e=>{if(!e.target.dataset.tag)return;const t=e.target.dataset.tag;state.tags.has(t)?state.tags.delete(t):state.tags.add(t);e.target.classList.toggle("active");saveFilters();render()};$("clearFilters").onclick=()=>{state.search=state.type=state.difficulty="";state.tags.clear();state.favOnly=state.wrongOnly=state.unsolvedOnly=false;$("searchBox").value="";$("typeFilter").value="";$("difficultyFilter").value="";$("favOnly").checked=false;$("wrongOnly").checked=false;$("unsolvedOnly").checked=false;document.querySelectorAll(".chip").forEach(c=>c.classList.remove("active"));saveFilters();render()};$("resetBtn").onclick=()=>{if(confirm("Tum ilerleme silinsin mi?")){state.progress={};save();render()}};render()}
...
assert counts.get("test", 0) >= 85, counts
...
"""
    ...
    final_report = f"""# Final Raporu

- Okunan toplam dosya sayisi: {len(inventory)}
- Ozgun cikarilan kaynak sayisi: {len(extracts)}
- Ozetlenen ana kaynaklar: Konu 1-15 PDFleri, programlama_dilleri PDFleri, Ders Sunumlari.zip proje raporlari, PROMPT.md ve ek HTML/MD kaynaklari
- Uretilen toplam soru sayisi: {len(questions)}
- Test soru sayisi: {counts.get('test', 0)}
- Klasik soru sayisi: {counts.get('klasik', 0)}
- Dil tanima soru sayisi: {counts.get('dil_tanima', 0)}
...
- Ogrenci icin onerilen ilk calisma yolu: 06 -> 01 -> 04 -> index.html filtreli soru cozumu -> yanlislar -> 07
"""
